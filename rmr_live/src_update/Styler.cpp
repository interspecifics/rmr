...
void Styler::update() {
	for (int i = 0; i < n; i++) {
		actual_radius_A[i] = ofLerp(actual_radius_A[i], next_radius_A[i], 0.06);
		actual_radius_B[i] = ofLerp(actual_radius_B[i], next_radius_B[i], 0.10);
		av_A[i] = ofLerp(av_A[i], nv_A[i], 0.2);
		av_B[i] = ofLerp(av_B[i], nv_B[i], 0.18);
	}
}
